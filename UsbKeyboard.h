...
void KeyDown(byte keyStroke, int num) {
  while (!usbInterruptIsReady()) {}
  reportBuffer[num] = keyStroke;
  usbSetInterrupt(reportBuffer, sizeof(reportBuffer));
}//Нажимает указанный символ и помещает информацию о нажатии в указанный слот
...
void KeyUp(int num) {
  ...
  reportBuffer[num] = 0;
  usbSetInterrupt(reportBuffer, sizeof(reportBuffer));
}//Поднимает символ из указанного слота

void KeyU(byte keyStroke) {
  for (int i = 1; i < BUFFER_SIZE; ++i){
    if (reportBuffer[i] == keyStroke)
    KeyUp(i);
  }
}//Поднимает указанный символ

int KeyIsDown(byte keyStroke) {
  for (int i = 1; i < BUFFER_SIZE; ++i){
    if (reportBuffer[i] == keyStroke)
    return 1;
  }
  return 0;
}//возвращает 1 если указанная символ нажат
///////////
...
void KeyUpAll() {
  ...
  memset(reportBuffer, 0, sizeof(reportBuffer));    
  usbSetInterrupt(reportBuffer, sizeof(reportBuffer));
}//Отпустить все кнопки(очистить буффер)
////////////////////////////////////////////////
